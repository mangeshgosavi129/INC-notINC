...
class TestDriverEndpoints:
    def test_driver_status(self, client):
        ...
        # Ensure EV is dispatched first so we see full status
        dispatch_resp = client.post(f"/api/ev/dispatch/{run_id}", json={
            "ev_id": "AMB_TEST", "vehicle_type": "ambulance", "origin_corridor": "CORR_01", "dispatch_time": 0.0
        })
        ...
        assert "destination_node" in data
    ...
    def test_arrival_comparison_endpoint(self, client):
        resp = client.post("/api/simulation/init", json={
            "name": "Arrival Test", "controller_type": "fixed_time", "duration_s": 3600.0,
        })
        run_id = resp.json()["run_id"]
        
        # Dispatch EV
        client.post(f"/api/ev/dispatch/{run_id}", json={
            "ev_id": "AMB_01", "vehicle_type": "ambulance"
        })
        
        # Run to completion so EV arrives
        client.post(f"/api/simulation/run/{run_id}")
        
        # Test arrival comparison endpoint
        resp = client.get(f"/api/driver/arrival-comparison/{run_id}")
