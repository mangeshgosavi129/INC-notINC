...
async def run_simulation(run_id: str):
    ...
    try:
        await run_in_threadpool(simulation_service.start_sync, run_id)
...
async def set_speed(run_id: str, req: SimSpeedRequest):
    try:
        speed = req.validated_speed
        simulation_service.set_speed(run_id, speed)
        return {"run_id": run_id, "speed": speed}
    except ValueError as e:
        raise HTTPException(404, str(e))
