...
def handle_agent_replan_trigger(state: SimulationState,
    ...
    # Additionally, apply direct preemption for the nearest intersection
    # when EV is active — this provides immediate response between replans
    from backend.app.controllers.preemption import should_activate_preemption, preempt_for_ev
    if should_activate_preemption(state):
        preempt_events = preempt_for_ev(state, event.scheduled_time)
        new_events.extend(preempt_events)
