...
class MCTSController:
    ...
    def decide(self, state: SimulationState,
        ...
        # When EV is active, focus search on the EV's route intersections
        # instead of all corridor intersections (dramatically improves search quality)
        active_ids = self.intersection_ids
        active_configs = self.intersection_configs
        if state.ev and state.ev.status.value not in ("idle", "arrived"):
            ev_corridor = state.ev_corridor or state.corridor
            route_ids = list(ev_corridor.intersection_ids)
            if len(route_ids) >= 2:
                active_ids = route_ids
                config_map = {c["intersection_id"]: c for c in self.intersection_configs}
                active_configs = [config_map[iid] for iid in route_ids if iid in config_map]

        # Build MCTS state from simulation
        mcts_state = MCTSState.from_simulation(state, sim_time)

        # Filter snapshots to active intersections only
        active_set = set(active_ids)
        filtered_snapshots = tuple(
            s for s in mcts_state.intersection_states
            if s.intersection_id in active_set
        )
        mcts_state = MCTSState(
            time=mcts_state.time,
            intersection_states=filtered_snapshots,
            ev_link_index=mcts_state.ev_link_index,
            ev_position_on_link=mcts_state.ev_position_on_link,
            ev_speed=mcts_state.ev_speed,
            ev_active=mcts_state.ev_active,
        )

        # Run search
        search = MCTSSearch(
            config=self.config,
            intersection_configs=active_configs,
            intersection_ids=active_ids,
            ev_phases=ev_phases,
        )
