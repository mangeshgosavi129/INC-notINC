...
class SimulationService:
    ...
    def init_simulation(self, name: str = "Unnamed Run",
        ...
        max_mins = config_service.simulation_config.get("max_time_minutes", 60)
        if max_mins > 0:
            duration_s = min(duration_s, float(max_mins * 60))
    ...
    def dispatch_ev(self, run_id: str, ev_id: str, vehicle_type: str,
                    ...
                    dispatch_time: float | None = None) -> None:
        sim = self._get_sim(run_id)
        t = dispatch_time if dispatch_time is not None else sim.sim_time
    ...
    async def _send_driver_update(self, run_id: str, state: SimulationState) -> None:
        ...
        directions = None
        if ev.current_link_index < len(state.corridor.links):
            link = state.corridor.links[ev.current_link_index]
            dist_remaining = max(0, link.length_meters - ev.position_on_link)
            total_remaining = dist_remaining
            for i in range(ev.current_link_index + 1, len(state.corridor.links)):
                total_remaining += state.corridor.links[i].length_meters
